...
def run():
    ...
    with app.app_context():
        ...
        records = utils.get_records(**app.config)
        extra = {'mixin': BaseMixin, 'get_name': partial(slugify, separator='_')}
        kwargs = merge([app.config, extra])
        job = partial(swutils.populate, db.engine, records=records, **kwargs)
